...
from core.serializers import UserSerializer, PublicidadSerializer, UserAdminSerializer, ReportSerializer, UserStatusSerializer, verificarFacturaSerializer, RecordBillSerializer, UserAdminClientSerializer, UserTestClientSerializer, ExpiredBillsSerializer, obtenerIDSerializer, GlobalVariablesSerializer
from rest_framework.permissions import IsAuthenticated, AllowAny
from rest_framework.authentication import TokenAuthentication
from rest_framework.decorators import api_view
from rest_framework.settings import api_settings
from rest_framework.decorators import permission_classes, authentication_classes
from .models import Administrador, Gerente, Operator, User, Cliente, Factura, Reporte, Publicidad, Globales
import requests
import json
    ...
    try:
        ...
        if user.is_client == True:
            ...
            facturasSinPagar = Factura.objects.filter(
                cliente_id=user_id, pagado=False)
            cantidadFacturasSinPagar = len(facturasSinPagar)
            tolerancia_meses = int(Globales.objects.filter(nombre='tol_meses')[0].valor)
            if cantidadFacturasSinPagar == 1:
                client.al_dia = True
                client.fecha_corte_energia = None
            elif cantidadFacturasSinPagar > tolerancia_meses+1:
                client.al_dia = False
            else:
                factura = Factura.objects.get(factura_id=request.data["factura_id"])
                client.fecha_corte_energia = client.fecha_corte_energia + relativedelta(months=1)
            client.save()
        else:
            return Response({"error": True}, status=status.HTTP_401_UNAUTHORIZED)
    except Cliente.DoesNotExist:
...
def get_client_factura(request, pk):
    user = Token.objects.get(key=request.auth.key).user
    if user.is_operator == True or user.is_client == True:
        try:
            ...
            if cantidadFacturasSinPagar == 1:
                al_dia = True
            elif cantidadFacturasSinPagar > tolerancia_meses:
                al_dia = False
...
def crear_factura(request, pk):
    ...
    body = '''
        ...
        En este correo se encuentra su factura de de energía eléctrica identificada con el número {bill_id}.

        Cualquier duda o inquietud, con mucho gusto será atendida por cualquiera de nuestros canales de atención.

        Electrica_ABC team
    '''.format(name = user_client.nombre, lastName = user_client.apellido, bill_id = factura.factura_id)
    ...
    pdfname = 'electrica_api/facturas_pdf/' + nombre_archivo_pdf
...
@authentication_classes([])
@permission_classes([])
def obtainListFactura(request):
    user = Factura.objects.all()
    serializer= obtenerIDSerializer(user, many=True)
    return Response(serializer.data)
...
# Clase para crear una variable global

class CreateGlobal(generics.CreateAPIView):
    # Debe cambiarse en un futuro AllowAny
    permission_classes = [AllowAny]
    serializer_class = GlobalVariablesSerializer

# Método para obtener todas las variables globales

@api_view(['GET'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def get_all_global_vars(request):
    user = Token.objects.get(key=request.auth.key).user
    if user.is_admin:
        globales = Globales.objects.all()
        serializer = GlobalVariablesSerializer(globales, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    else: 
        return Response({"error": True, "error_cause": "only Admins are allowed to view global variables!"}, status=status.HTTP_401_UNAUTHORIZED)

# Método para obtener o modificar una sola variable global

@api_view(['GET', 'PUT'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def get_put_one_global_vars(request, val):
    try:
        user = Token.objects.get(key=request.auth.key).user
        if user.is_admin:
            if request.method == "GET":
                globales = Globales.objects.filter(nombre = val)
                serializer = GlobalVariablesSerializer(globales, many=True,
                    context={ "request": request })
                return Response(serializer.data, status=status.HTTP_200_OK)
            elif request.method == "PUT":
                globales_obj = Globales.objects.get(nombre = val)
                serializer = GlobalVariablesSerializer(globales_obj,
                    data = request.data,
                    context = { "request": request })
                if serializer.is_valid():
                    serializer.save()
            ...
            return Response({"error": True, "error_cause": "only Admins are allowed to view global variables!"}, status=status.HTTP_401_UNAUTHORIZED)
    except Globales.DoesNotExist:
        return Response({"error": True, "error_cause": "global variable not found!"}, status=status.HTTP_404_NOT_FOUND)

# Método para borrar una variable global.

@api_view(['DELETE'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def del_global_var(request, val):
    try:
        user = Token.objects.get(key=request.auth.key).user
        if user.is_admin:
            globales = Globales.objects.get(nombre = val)
            globales.delete()
            return Response({ "detail": "Global variable successfully removed!" }, status=status.HTTP_200_OK)
        else: 
            return Response({"error": True, "error_cause": "only Admins are allowed to view global variables!"}, status=status.HTTP_401_UNAUTHORIZED)
    except Globales.DoesNotExist:
            return Response({"error": True, "error_cause": "global variable not found!"}, status=status.HTTP_404_NOT_FOUND)

# Método para que un operador o gerente obtengan todas las publicidades existentes

@api_view(['GET'])
@authentication_classes([TokenAuthentication])
@permission_classes([IsAuthenticated])
def get_all_commerces(request):
    user = Token.objects.get(key=request.auth.key).user
    if user.is_gerente or user.is_operator:
        publicidad = Publicidad.objects.all()
        serializer = PublicidadSerializer(publicidad, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    else: 
        return Response({"error": True, "error_cause": "only Managers or Operators are allowed to view commerces!"}, status=status.HTTP_401_UNAUTHORIZED)

# Método para mostrar todas las publicidades
...
# Método para que el gerente u operador vea el estado financiero de los clientes
...
def get_financial_status(request):
    ...
    if user.is_gerente or user.is_operator:
        ...
        for i in range(len(cliente)):
            ...
            variable = 0
            facturas_cliente_sin_pagar = Factura.objects.filter(
                cliente_id=cli.user.pk, pagado=False).order_by('-factura_id')
            # Se encuentran las facturas que no han sido pagadas hasta la fecha y se suma su valor
            if len(facturas_cliente_sin_pagar) > 0:
                variable = 1
                for j in range(len(facturas_cliente_sin_pagar)-1):
                    pagos_acumulados += facturas_cliente_sin_pagar[variable].total_pagar
                    variable = variable + 1
            if cli.fecha_corte_energia == None:
                fecha_corte = date.today()
            elif type(cli.fecha_corte_energia).__name__ == "date":
                fecha_corte = cli.fecha_corte_energia
            else:
                fecha_corte = datetime.strptime(cli.fecha_corte_energia,'%Y-%m-%d %H:%M:%S').date()
            financial_status.append({
                'cliente_id': cli.user.pk, 
                'fecha_corte_energia': cli.fecha_corte_energia, 
                'dias_mora': abs(date.today()-fecha_corte), 
                'pagos_acumulados': str(pagos_acumulados),
                'cantidad_facturas_no_pagadas': str(variable),
            })
        return Response(financial_status, status=status.HTTP_200_OK)            
    else: 
        return Response({"error": True, "error_cause": "only Managers or Operators are allowed to view commerces!"}, status=status.HTTP_401_UNAUTHORIZED)