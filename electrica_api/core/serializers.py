...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "tipo", "password", "nombre","apellido",
                  "cedula", "email", "direccion", "fecha_nacimiento", 
                  "celular", "salario", "al_dia", "servicio_activo",
                  )
...
class UserAdminClientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cliente
        fields = ("user", "servicio_activo", "al_dia","fecha_corte_energia")
...
class RecordBillSerializer(serializers.ModelSerializer):
    ...
    cedula = serializers.SerializerMethodField('get_cedula')
    celular = serializers.SerializerMethodField('get_celular')

    def get_nombre(self, value):
        cli_id = getattr(value, 'cliente_id')
        return cli_id.user.nombre

    def get_cedula(self, value):
        cli_id = getattr(value, 'cliente_id')
        return cli_id.user.cedula

    def get_celular(self, value):
        cli_id = getattr(value, 'cliente_id')
        return cli_id.user.celular

    def create(self, validated_data):
        factura = Factura.objects.create(**validated_data)
        tolerancia_meses = int(Globales.objects.filter(nombre='tol_meses')[0].valor)
        cli_id = factura.cliente_id
        if cli_id.fecha_corte_energia == None:
            cli_id.fecha_corte_energia = factura.fecha_expedicion + relativedelta(months=tolerancia_meses)
        facturasSinPagar = Factura.objects.filter(
            cliente_id=cli_id, pagado=False)
        cantidadFacturasSinPagar = len(facturasSinPagar)
        if cantidadFacturasSinPagar > tolerancia_meses:
            cli_id.al_dia = False
        cli_id.save()
        return factura
    ...
    class Meta:
        ...
        fields = ('factura_id', 'cliente_id','operador_id','fecha_expedicion',
                  ...
                  'estrato_economico','consumo_en_kwh', 'total_pagar', 'nombre',
                  'celular', 'cedula')
...
class obtenerIDSerializer(serializers.ModelSerializer):
...
class GlobalVariablesSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        globalVar = Globales.objects.create(nombre=nombre, valor=valor)
        return globalVar
